...
public class ScoreModifyAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * String[] scoreId = request.getParameterValues("scoreId"); String scoreIndivi
		 * = ""; request.setAttribute("scoreId", scoreId);
		 * 
		 * for(int i = 0; i< scoreId.length; i++) { scoreIndivi = scoreId[i];
		 * 
		 ...
		 * 
		 * System.out.println( "scoreId:" + scoreIndivi + " subId:"+subId +
		 * " scoreDate:" + scoreDate + " acqScore:" + acqScore);
		 * 
		 * ScoreVO scoVo = new ScoreVO(); scoVo.setSubId(subId);
		 * scoVo.setScoreDate(scoreDate); scoVo.setAcqScore(acqScore);
		 * scoVo.setScoreId(scoreIndivi);
		 * 
		 * ScoreDao scoDao = ScoreDao.getInstance(); scoDao.updateScore(scoVo);
		 * 
		 * 
		 * 
		 * }
