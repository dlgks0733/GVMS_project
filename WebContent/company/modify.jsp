				...
				<form role="form" method="post" onsubmit="return validateEmptyVal()" class="form-horizontal" enctype="multipart/form-data">
						...
						<div class="form-group input-short">
							...
							<div class="col-sm-10">
								<input type="text" name='compname' class="form-control"
									placeholder="기관 이름을 입력해주세요" value="${company.compname}">
							</div>
						</div>
						<div class="form-group input-short">
							<label for="exampleInputEmail1" class="col-sm-2 control-label">분야 <span class="must-mark">*</span></label> 
							<div class="col-sm-10">
								<select name="division" class="form-control">
									<option value="1"
										<c:if test="${company.division eq 1}">selected</c:if>>
										연구소
									</option>								
									<option value="2"
										<c:if test="${company.division eq 2}">selected</c:if>>
										교육센터
									</option>
								</select>
							</div>
						...
						<div class="form-group input-long">
							...
							<div class="col-sm-10">
								<input name='compfield' class="form-control"
									placeholder="기관의 분야를 입력해주세요" value="${company.compfield}">
							</div>
						</div>
						<div class="form-group input-long">
							...
							<div class="col-sm-10">
								<input name='target' class="form-control"
									placeholder="기관의 교육 또는 연구 대상을 입력해주세요" value="${company.target}">
							</div>
						</div>
						<div class="form-group input-short">
							...
							<div class="col-sm-10">
								<input name='phone' class="form-control" maxlength = '13'
									placeholder="전화번호를 입력해주세요" value="${company.phone}">
							</div>
						</div>
						<div class="form-group input-long">
							...
							<div class="row col-md-10">
								<div class="col-md-3 col-xs-4">
									<input name='zipcode' class="form-control" placeholder="우편번호"
										readonly value="${company.zipcode}">
								</div>
						...
						<div class="form-group input-short">
							...
							<div class="col-sm-10">
								<input name='frontaddr' class="form-control" placeholder="주소"
									readonly value="${company.frontaddr}">
							</div>
						</div>
						<div class="form-group input-short">
						...
						<div class="form-group input-short">
							...
							<div class="col-sm-10">
								<textarea rows="5" name='compdesc' class="form-control"
									placeholder="기관에 대한 상세한 설명을 입력해주세요">${company.compdesc}</textarea>
							</div>
						</div>
						<div class="form-group input-short">
					...
					<div class="box-footer btn-group-center">
						<button type="button" class="btn btn-danger btn-remove"
							onclick="location.href='remove?compnum=${company.compnum}'">삭제</button>
						<button type="submit" class="btn btn-primary"
							onclick="return submitViewData()">수정</button>
...
//뷰가 로드될 때 실행하는 함수
var compnum = ${company.compnum};
var template = Handlebars.compile($("#template").html());

$.getJSON("/admin/company/getAttach/"+compnum, function(list){
	$(list).each(function(){
...
//         	// 태그를 저장하는 문자열 변수
//             var str = "";
//         	// 이미지링크의 값을 경로 + 파일이름으로 넣어줌
//             document.getElementsByName("imglink")[0].value = getImageLink(data);
        	
//         	// 이미지타입을 검사후 이미지파일을 태그로 만듦
//             if (checkImageType(data)) {
//                 str = "<img src='displayFile?fileName=" + getImageLink(data) + "'/>";
//                 // imgHolder div 태그 안에 이미지 태그를 넣어줌
//                 document.getElementById("imgHolder").innerHTML = str;
//             } else {
//                 str = "<img src='displayFile?fileName=" + getImageLink(data) + "'/>";
//                 document.getElementById("imgHolder").innerHTML = str;
